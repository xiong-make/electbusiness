<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>addGoods.jsp</title>
    <link href="css/common.css" type="text/css" rel="stylesheet">
</head>
<body>
<form action="admin_addGoods?act=add" method="post" enctype="multipart/form-data">
    <table>
        <caption>
            <span class="title">添加商品</span>
        </caption>
        <tr>
            <td>商品名称<span class="xin">*</span></td>
            <td><input type="text" name="gname"/></td>
        </tr>
        <tr>
            <td>商品原价<span class="xin">*</span></td>
            <td><input type="text" name="goprice"></td>
        </tr>
        <tr>
            <td>商品折扣价<span class="xin">*</span></td>
            <td><input type="text" name="grprice"></td>
        </tr>
        <tr>
            <td>商品数量<span class="xin">*</span></td>
            <td><input type="text" name="gstore"></td>
        </tr>
        <tr>
            <td>商品图片<span class="xin">*</span></td>
            <td><input type="file" id="gpicture" name="gpicture"></td>
        </tr>
        <tr>
            <td>商品类型<span class="xin">*</span></td>
            <td>
                <select name="typeid">
                    <c:forEach var="gtp" items="${goodsType}">
                        <option value="${gtp.id}">${gtp.typename}</option>
                    </c:forEach>
                </select>
            </td>
        </tr>
        <tr>
            <td align="center"><input type="submit" value="提交"/></td>
            <td align="left"><input type="reset" value="重置"/></td>
        </tr>
    </table>
</form>
</body>
</html>
